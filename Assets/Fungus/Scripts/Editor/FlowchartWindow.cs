    ...
    public class FlowchartWindow : EventWindow
    {
        ...
        protected virtual void DrawOverlay(Event e)
        {
            ...
            GUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                var newString = EditorGUILayout.TextField(searchString, GUI.skin.FindStyle("ToolbarSearchTextField"), GUILayout.Width(150));
            ...
            GUILayout.BeginHorizontal();
            {
                GUILayout.BeginVertical(GUILayout.Width(440));
                {
                    ...
                    flowchart.VariablesScrollPos = GUILayout.BeginScrollView(flowchart.VariablesScrollPos);
                    {
                        ...
                        // Cria um editor para o flowchart e desenha a interface de usuário das variáveis
                        FlowchartEditor flowchartEditor = Editor.CreateEditor(flowchart) as FlowchartEditor;
                        if (flowchartEditor != null)
                        {
                            // Aqui chamamos DrawVariablesGUI com os parâmetros necessários
                            flowchartEditor.DrawVariablesGUI(true, 0);
                            DestroyImmediate(flowchartEditor);
                        }
                        else
                        {
                            // Tratamento de erro ou mensagem de aviso
                            GUILayout.Label("FlowchartEditor could not be instantiated.");
                        }
